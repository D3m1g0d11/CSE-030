...
class HTable {
        struct data *dt; //the table to hold hashed data structs
...
public:
        ...
        int add(struct data &d);   //adda a data pair into the table according to its key
        int remove(struct data &d); //removes data pair from the table according to its key
